...
// Generate HTML for the star ratings
function generateRating(rating) {
    let starsHtml = '';
    // Loop 5 times to build the 5 stars
    for (let i = 1; i <= 5; i++) {
        if (i <= rating) {
...
// Build the HTML template for a single recipe
...
function renderRecipes(recipeList) {
    ...
    // Map over the list, convert each to HTML, and join them into a single string
    const htmlStrings = recipeList.map(recipeTemplate).join('');
    mainElement.innerHTML = htmlStrings;
}

// Get a random recipe and render it on page load
function init() {
    // Math.random() gets a number between 0 and 1. Multiply by the array length and round down.
    const randomIndex = Math.floor(Math.random() * recipes.length);
    const randomRecipe = recipes[randomIndex];
    
    // The render function expects an array, so we wrap our single recipe in brackets
...
function filterRecipes(query) {
    ...
    const filtered = recipes.filter(recipe => {
        ...
        // Check if ANY of the tags include the search term
        const inTags = recipe.tags.some(tag => tag.toLowerCase().includes(lowerCaseQuery));
        
        return inName || inDescription || inTags;
    });

    // 2. Sort Alphabetically by Recipe Name
...
function searchHandler(event) {
    event.preventDefault(); // Prevents default page refresh behavior
...
// Listen for the "Enter" key inside the input field
